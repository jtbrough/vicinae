  ...
  in {
    packages = forEachPkgs (pkgs: {
      ...
      nix-update-script = pkgs.writeShellScriptBin "nix-update-script" ''
        OLD_API_DEPS_HASH=$(${pkgs.lib.getExe pkgs.nix} eval --raw .#packages.${pkgs.system}.default.apiDeps.hash)
        OLD_EXT_MAN_DEPS_HASH=$(${pkgs.lib.getExe pkgs.nix} eval --raw .#packages.${pkgs.system}.default.extensionManagerDeps.hash)
